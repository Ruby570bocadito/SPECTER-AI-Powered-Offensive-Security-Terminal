...
class SpecterEngine:
    ...
    async def initialize(self) -> None:
        ...
        self.agent_orchestrator = SmartOrchestrator(cmd_executor=self._run_shell_command)
    ...
    async def _process_natural_language(self, user_input: str) -> None:
        """Procesa input conversacional con el LLM"""
        ...
        try:
            ...
            if not client.connect():
                self.console.print("[yellow]LLM no disponible. ¿Está Ollama corriendo?[/]")
                return
            ...
            response = await self._generate_with_cancel(
                lambda: client.generate(user_input, system_prompt, stream=False),
                "Procesando..."
            )
            ...
            if response and response.strip():
                ...
                await self._execute_llm_commands(response, client, system_prompt)
    ...
    async def _generate_with_cancel(self, generate_func, label: str) -> str:
        """Genera respuesta con posibilidad de cancelación"""
        import threading
        import time
        ...
        def check_esc_key():
            """Monitorea teclado para Esc en background"""
            try:
        ...
        with Progress(
            ...
            TextColumn(f"[bold #00D4FF]{label}[/]"),
            TextColumn("[#555555](Ctrl+C para cancelar)[/]"),
            TimeElapsedColumn(),
    ...
    async def process_interactive_input(self, user_input: str) -> None:
        ...
        try:
            ...
            if not client.connect():
                self.console.print("[yellow]LLM no disponible[/]")
                return
            ...
            with Progress(
                SpinnerColumn(spinner_name="dots2", style="#00FF88"),
                TextColumn("[bold #00D4FF]Procesando...[/]"),
                TimeElapsedColumn(),
                console=self.console,
                transient=True,
            ) as progress:
                ...
                response = await loop.run_in_executor(
                    None,
                    lambda: client.generate(interactive_prompt, system_prompt, stream=False),
                )
            ...
            if continue_prompt and output:
                ...
                with Progress(
                    ...
                    analysis = await loop.run_in_executor(
                        None,
                        lambda: client.generate(continue_prompt, system_prompt, stream=False),
                    )
                ...
                if analysis and analysis.strip():
                    ...
                    await self._execute_llm_commands(analysis, client, system_prompt)
    ...
    def _ask_next_action(self) -> tuple[str, str]:
        ...
        suggestions = (
            "[dim]Selecciona una opción o escribe libremente:[/]\n\n"
        ...
        return ("interactive", choice_map.get(choice, "continuar"))
    ...
    async def _run_shell_command(self, cmd: str) -> tuple[str, str, int]:
        ...
        try:
            ...
            return "", "Timeout: el comando tardó más de 60 segundos.", -1
        except Exception as exc:
...
"""
    ...
    async def _switch_model(self, model_name: str) -> None:
        ...
        self.console.print(Panel.fit(
            ...
            f"Activo:   [bold #00D4FF]{markup_escape(model_name)}[/]\n\n"
